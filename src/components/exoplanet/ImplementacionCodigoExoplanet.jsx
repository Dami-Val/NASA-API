...
function ImplementacionCodigoExoplanet() {
  return (
    <section className="code-section">
      <h2>Código de Implementación Exoplanet</h2>
      <p>Aquí está el código JavaScript utilizado para interactuar con la API de Exoplanetas de NASA en esta demostración:</p>
      <pre>
{`/**
 * Funciones para interactuar con la API de exoplanetas de NASA
 * La API utiliza el protocolo Table Access Protocol (TAP) que permite realizar consultas SQL
 */
 ...
 */
async function consultarExoplanetas(consulta, limite = 100, formato = 'json') {
    try {
 ...
 */
async function consultarExoplanetasPorFiltros(filtros, ordenamiento = { campo: 'pl_name', direccion: 'ASC' }, limite = 100) {
 ...
 */
async function obtenerDetallesExoplaneta(nombrePlaneta) {
    const consulta = \`
        SELECT * FROM ps 
 ...
 */
async function consultarEstadisticasExoplanetas() {
    try {
...
// Ejemplo: Obtener exoplanetas en la zona habitable
consultarExoplanetas("SELECT pl_name, hostname, pl_orbper, pl_rade, st_dist, pl_eqt FROM ps WHERE pl_eqt BETWEEN 180 AND 310 AND pl_rade < 2 ORDER BY pl_eqt ASC LIMIT 10")
    .then(datos => {
        console.log('Exoplanetas potencialmente habitables:', datos);
    })
    .catch(error => {
        console.error('Error:', error);
    });

// Ejemplo: Obtener detalles de un exoplaneta específico
obtenerDetallesExoplaneta('TRAPPIST-1 e')
    .then(planeta => {
        console.log('Detalles de TRAPPIST-1 e:', planeta);
    })
    .catch(error => {
        console.error('Error:', error);
    });
...
consultarEstadisticasExoplanetas()
    .then(estadisticas => {
        console.log('Estadísticas de exoplanetas:', estadisticas);
    })
    .catch(error => {
        console.error('Error:', error);
    });`}
...
export default ImplementacionCodigoExoplanet;